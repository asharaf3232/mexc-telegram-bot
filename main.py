...
try:
    ...
    logging.warning("Libraries 'nltk' or 'investpy' not found. Fundamental analysis will be disabled.")
...
EDITABLE_PARAMS = {
    ...
    "الفلاتر والاتجاه": [
        ...
        "fundamental_analysis_enabled" # [UPGRADE] New setting
    ]
}
PARAM_DISPLAY_NAMES = {
    ...
    "fundamental_analysis_enabled": "فلتر الأخبار والبيانات", # [UPGRADE] New display name
}
...
DEFAULT_SETTINGS = {
    ...
    "market_regime_filter_enabled": True, "fundamental_analysis_enabled": True, # [UPGRADE] Added new setting
...
# --- [UPGRADE] قسم التحليل الأساسي والأخباري --- #
def get_high_impact_economic_events():
    """
    تجلب الأحداث الاقتصادية ذات التأثير العالي لليوم الحالي.
    """
    try:
        ...
        if not today_events.empty:
            logging.warning(f"High-impact events today: {today_events['event'].tolist()}")
            return today_events['event'].tolist()
        return []
    except Exception as e:
        logging.error(f"Failed to fetch economic calendar data: {e}")
        return None

def get_latest_crypto_news(limit=15):
    """
    تجلب آخر عناوين الأخبار من مصادر مختلفة.
    """
    urls = [
...
def analyze_sentiment_of_headlines(headlines):
    """
    تحلل قائمة من العناوين الإخبارية وتعطي درجة مشاعر إجمالية.
    """
...
async def get_fundamental_market_mood():
    """
    تحدد مزاج السوق العام بناءً على الأخبار الاقتصادية ومشاعر الأخبار.
    """
...
async def worker(queue, results_list, settings, failure_counter):
    while not queue.empty():
        ...
        try:
            ...
            if len(ohlcv) < max(liq_filters['rvol_period'] + 5, ema_filters.get('ema_period', 200), vol_filters.get('atr_period_for_filter', 14) + 5):
            ...
            # --- [CRITICAL FIX] ---
            # Corrected the typo from iloc[-2'] to iloc[-2]
    ...
    async with scan_lock:
        ...
        if settings.get('market_regime_filter_enabled', True):
            ...
            if not is_market_ok:
                ...
                await send_telegram_message(context.bot, {'custom_message': f"⚠️ *إيقاف الفحص: وضع السوق الفني غير مناسب*\n- السبب: {reason}", 'target_chat': TELEGRAM_CHAT_ID})
                return
    ...
    for result in filter(None, results):
        ...
        if status in ['ناجحة', 'فاشلة']:
            ...
            try:
                ...
                duration = datetime.now(EGYPT_TZ) - entry_time.replace(tzinfo=EGYPT_TZ)
                days, rem = divmod(duration.total_seconds(), 86400)
                hours, rem = divmod(rem, 3600)
                minutes, _ = divmod(rem, 60)
                parts = [f"{int(d)}ي" for d in [days, hours, minutes] if d > 0]
                duration_str = " ".join(parts) if parts else "< 1د"
            except: pass
            ...
            message = (
                f"{icon} *إغلاق صفقة #{original_trade['id']}* {icon}\n\n"
                f"▫️ *العملة:* `{original_trade['symbol']}`\n"
                f"▫️ *السبب:* {reason_str}\n"
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE): await update.message.reply_text("أهلاً بك في بوت المحلل الآلي! (v26.1 - Fixed)", reply_markup=ReplyKeyboardMarkup(main_menu_keyboard, resize_keyboard=True))
...
async def show_parameters_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for category, params in EDITABLE_PARAMS.items():
        ...
        rows = [params[i:i + 2] for i in range(0, len(params), 2)]
        for row in rows:
            button_row = []
...
async def main_text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if 'awaiting_input_for_param' in context.user_data:
        ...
        try:
            ...
            context.user_data['awaiting_input_for_param'] = param
            context.user_data['settings_menu_id'] = settings_menu_id
            if settings_menu_id:
...
async def post_init(application: Application):
    ...
    await application.bot.send_message(chat_id=TELEGRAM_CHAT_ID, text=f"🚀 *المحلل الآلي جاهز للعمل! (v26.1)*", parse_mode=ParseMode.MARKDOWN)
...
if __name__ == '__main__':
    try:
        ...
        logging.critical(f"Bot stopped due to a critical error: {e}", exc_info=True)
